...
def examine_run_artifacts(wfr_id, u):
    ...
    # XXX: if the metadata file doesn't appear some time after the wfr finished,
    # that that suggests something went wrong in internally in scallywag, before
    # the it write it, in which case we should change the status to errored

    # if we didn't find the metadata file in the workflow artifacts, try again
    # later
